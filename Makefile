...
dist-win32: 
	...
	rm -f /tmp/game.zip
	cp winlibs/lib/*dll .
	cd .. && zip -r /tmp/game-win32.zip game/*.dll game/game.exe game/wav game/img game/README
	rm *dll

dist-linux: 
	$(MAKE) clean
	$(MAKE)
	cd .. && tar -zcvf /tmp/game-linux.tgz game/game game/wav game/img game/README

dist-src:
	$(MAKE) clean
	cd .. && tar -zcvf /tmp/game-src.tgz game/*.c game/*.h game/Makefile game/img game/wav game/README
